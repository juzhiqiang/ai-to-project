在 services/api 中实现完整的 UI 交互闭环，严格按以下要求执行：

1. 交互状态机：
   - 新建 services/api/src/llm/ui-protocol/ui-flow.service.ts
   - 实现需求分析流程的状态机：
...
2. Action 处理器：
   - 新建 services/api/src/llm/ui-protocol/ui-action.handler.ts
   - 根据 UIAction 的 componentType 和 payload 分发处理
   - 每次处理后更新 context（sessionStage + collectedData）
   - 返回下一阶段的 AIUIResponse

3. 更新路由：
   - POST /api/ui-api/action：接收 { sessionId, action }，返回下一阶段的 AIUIResponse
